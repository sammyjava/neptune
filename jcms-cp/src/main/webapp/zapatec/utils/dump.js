// $Id: dump.js 15736 2009-02-06 15:29:25Z nikolai $
/*
 * <pre>
...
dump.getHTML = function(root, outputRoot, intDeep) {
	...
	switch (root.nodeType) {
		...
		case 11: // Node.DOCUMENT_FRAGMENT_NODE
			...
			if (Zapatec.is_ie && root_tag == "head") {
				if (outputRoot)
					html += "\n<head>\n";
				// lowercasize
			...
			if (outputRoot && !closed) {
				for (i2=0; i2 < intDeep; i2++) html+="  "
				html += "</" + root.tagName.toLowerCase() + ">";
				if (0 != i2 || !outputRoot) html += "\n";
			}
			break;
		case 3: // Node.TEXT_NODE
			// If a text node is alone in an element and all spaces, replace it with an non breaking one
			// This partially undoes the damage done by moz, which translates '&nbsp;'s into spaces in the data element
			var textHtml = /^script|style$/i.test(root.parentNode.tagName) ? root.data : this.htmlEncode(root.data);
